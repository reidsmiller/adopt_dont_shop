...
RSpec.describe AppPet, type: :model do
  ...
  describe 'instance methods' do
    before(:each) do
      ...
      @pet_4 = @shelter_2.pets.create!(adoptable: false, age: 3, breed: 'doberman', name: 'Lobster')
    ...
    it 'can check app_pet status and pet adoptable status and send a status code' do
      ...
      expect(AppPet.match_pet_to_app_pet(@pet_1)).to eq("Pending")
      expect(AppPet.match_pet_to_app_pet(@pet_2)).to eq("Approved")
      expect(AppPet.match_pet_to_app_pet(@pet_3)).to eq("Rejected")
      expect(AppPet.match_pet_to_app_pet(@pet_4)).to eq("Adopted")
    end
  end
end